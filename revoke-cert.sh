...
echo "Revoking certificate : $CERT.crt:"
#openssl ca -config ca.config -out $CERT.crt -infiles $CERT.crt
openssl ca -config ca.config -keyfile ca.key -cert ca.crt -revoke $CERT.crt
